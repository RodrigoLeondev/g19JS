...
/*   const filterUsers = ( strToFilter ) => {
    users.filter(cv => {
      if(cv.name === strToFilter | 
          cv.username === strToFilter |
            cv === '@')
      return true
    })
  }
...
/*  Codigo Rodrigo
  ...
  const userPrint = () => { 
    let template = users.reduce((acc, cv, index, arr) => {
        ...
        return acc += ` 
          ...
          <a href="#" class="list-group-item list-group-item-action " aria-current="true"> 
              ...
              <small ${index + 1} ${ index >= 1 ? 'day' : 'days'}> Days ago </small>
              </div> 
              <p class="mb-1">${cv.body}</p> 
              <small>${posts.id}</small> 
          </a> 
        </div> ` }, '') 
        document.querySelector('#filter__user').innerHTML = template 
      } 
        document.addEventListener('DOMContentLoaded', userPrint)
...
inputSearch.addEventListener('keyup', () => {
  ...
  // Agregarlos al leyout en #lista__users
}) */
